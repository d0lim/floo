...
// LoggingProxy는 다른 프록시를 감싸서 로깅 기능을 추가하는 래퍼 구현체입니다.
type LoggingProxy struct {
	Wrapped HTTPProxy
}

// NewLoggingProxy는 기존 프록시를 감싸는 로깅 프록시를 생성합니다.
func NewLoggingProxy(wrapped HTTPProxy) *LoggingProxy {
	return &LoggingProxy{
		Wrapped: wrapped,
	}
}
...
func (p *LoggingProxy) Proxy(c *fiber.Ctx, upstream string) error {
	start := time.Now()
